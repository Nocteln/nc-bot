...
module.exports = {
    ...
    async run(bot, message, args) {
        ...
        try {
            ...
            if(messages.length <= 0) return message.reply({embeds: [embedr("Red", "❌ erreur",`Aucun message à supprimer car ils datent de plus de 14 jours`)]})
            
            await channel.bulkDelete(messages)
            await message.reply({embeds: [embedr("Green", ":white_check_mark: succes",`${messages.size} messages suprimés car les autres dataient de plus de 14 jours`)]})
        }
