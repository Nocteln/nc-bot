...
module.exports = {
    ...
    options: [
        {
            type: 'user',
            name: "utilisateur",
            description: "Le membre dont vous voulez retirer des niveax",
            required: true,
            autocomplete: false,
        },
        {
            type: 'number',
            name: 'quantité',
            description: "le nombre de niveaux à retirer",
            required: false,
            autocomplete: false,
        },
    ],
    ...
    async run(bot, message, args, db) {
        ...
        db.query(`SELECT * FROM xp WHERE guild = '${message.guildId}' AND user = '${user.id}'`, async (err,req) => {
            ...
            if(level < nombre ) return message.reply({embeds: [embedr("Red", "❌ erreur",`le membre n'as que ${level} niveaux`)]})
            if(req.length <1 ) return message.reply({embeds: [embedr("Red", "❌ erreur","Ce membre n'as pas d'xp!")]})
            const xpremove = level-nombre

            db.query(`UPDATE xp SET level = ${xpremove} WHERE guild = '${message.guildId}' AND user = '${user.id}'`)
            await message.reply({embeds: [embedr("Green",":white_check_mark: succes",`${nombre} niveaux retiré à ${user.tag}, il maintenant level ${xpremove} avec ${xp} xp \n pour retirer de l'experience utilisez la comande </remove_xp:0>`)]})
        })
